...
var HASH_NUMBER_START_X = 400;
var HASH_X_DIFF = 15;
var HASH_NUMBER_START_Y = 10;
var HASH_ADD_START_Y = 30;
var HASH_INPUT_START_X = 200;
var HASH_INPUT_X_DIFF = 14;
...
Hash.prototype.addControls = function()
{
	...
	var radioButtonList = addRadioButtonGroupToAlgorithmBar(["Хешувати цілі числа", "Хешувати строку"], "HashType");
...
Hash.prototype.changeHashType = function(newHashingIntegerValue)
{
	...
	if (this.hashingIntegers)
	{
		...
		this.insertField.onkeydown = this.returnSubmit(this.insertField,  this.insertCallback.bind(this),
			MAX_HASH_LENGTH, true);
		this.deleteField.onkeydown = this.returnSubmit(this.insertField,  this.deleteCallback.bind(this),
			MAX_HASH_LENGTH, true);
		this.findField.onkeydown = this.returnSubmit(this.insertField,  this.findCallback.bind(this),
			MAX_HASH_LENGTH, true);
	}
	else
	{
...
Hash.prototype.doHash = function(input)
{
	...
	else
	{
		...
		for (var i = 0; i < input.length; i++)
		{
			...
			this.cmd("CreateLabel", wordToHashID[i], wordToHash[i], HASH_INPUT_START_X + i * HASH_INPUT_X_DIFF,
				HASH_INPUT_START_Y, 0);
		}
		...
		for (i = wordToHash.length-1; i >= 0; i--)
		{
			for (j = 0; j < 32; j++)
			{
				this.cmd("CreateLabel", digits[j],hashValue[j], HASH_NUMBER_START_X + j * HASH_X_DIFF,
					HASH_NUMBER_START_Y, 0);
			}
			...
			for (var j = 7; j >= 0; j--)
			{
				...
				this.cmd("CreateLabel", nextByteID[j], nextByte[j], HASH_INPUT_START_X + i*HASH_INPUT_X_DIFF,
					HASH_INPUT_START_Y, 0);
				this.cmd("Move", nextByteID[j], HASH_NUMBER_START_X + (j + 24) * HASH_X_DIFF, HASH_ADD_START_Y);
			}
			this.cmd("Step");
			this.cmd("CreateRectangle", barID, "", 32 * HASH_X_DIFF, 0, HASH_NUMBER_START_X, HASH_ADD_LINE_Y,
				"left","bottom");
			...
			for (j = 0; j < 32; j++)
			{
				this.cmd("CreateLabel", resultDigits[j], hashValue[j], HASH_NUMBER_START_X + j * HASH_X_DIFF,
					HASH_RESULT_Y, 0);
			}
			...
			if (i > 0)
			{
				...
				for (j = 0; j < 4; j++)
				{
					this.cmd("Move", resultDigits[j], HASH_NUMBER_START_X + (j + ELF_HASH_SHIFT) * HASH_X_DIFF,
						HASH_ADD_START_Y);
					hashValue[j+28] = 0;
					this.cmd("CreateLabel", floatingDigits[j],0, HASH_NUMBER_START_X + (j + 28) * HASH_X_DIFF,
						HASH_NUMBER_START_Y,0);
					if (floatingVals[j])
					{
