...
ClosedHashBucket.prototype.findElement = function(elem)
{
	...
	if (index == -1)
	{
		this.cmd("SetText", this.ExplainLabel, "Елемент " + elem + " не знайдено");
	}
	else
	{
		this.cmd("SetText", this.ExplainLabel, "Елемент " + elem + " знайдено");
	}
	return this.commands;
}
...
ClosedHashBucket.prototype.setup = function()
{
	...
	for (i = 0; i <= NUM_BUCKETS; i++)
	{
		...
		if (i == NUM_BUCKETS)
		{
			this.cmd("CreateLabel", nextID, "Поза масивом", nextXPos + 3, nextYPos + ARRAY_ELEM_HEIGHT / 2 , 0);
		}
		else
		{
