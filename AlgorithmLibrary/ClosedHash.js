...
ClosedHash.prototype.addControls = function()
{
	...
	var radioButtonList = addRadioButtonGroupToAlgorithmBar(["Лінійне опробування: f(i) = i",
															 "Квадратичне опробування: f(i) = i * i",
															"Подвійне хешування: f(i) = i * hash2(elem)"],
															"CollisionStrategy");
...
ClosedHash.prototype.insertElement = function(elem)
{
	...
	this.cmd("SetText", this.ExplainLabel, "Вставка елементу: " + String(elem));
...
{
	this.commands = new Array();
	this.cmd("SetText", this.ExplainLabel, "Видалення елементу: " + elem);
	var index = this.doHash(elem);
	...
	if (index > 0)
	{
		this.cmd("SetText", this.ExplainLabel, "Видалення елементу: " + elem + "  Елемент видалено");
	...
	{
		this.cmd("SetText", this.ExplainLabel, "Видалення елементу: " + elem + "  Елемент знаходится поза таблицею");
	}
	return this.commands;
...
ClosedHash.prototype.findElement = function(elem)
{
	...
	this.cmd("SetText", this.ExplainLabel, "Пошук елементу: " + elem);
	var index = this.doHash(elem);

	var found = this.getElemIndex(index, elem) != -1;
	if (found)
	{
		this.cmd("SetText", this.ExplainLabel, "Пошук елементу: " + elem+ "  Знайдено!")
	}
	else
	{
		this.cmd("SetText", this.ExplainLabel, "Пошук елементу: " + elem+ "  Не знайдено!")
